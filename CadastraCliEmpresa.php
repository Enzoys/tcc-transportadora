...
if ($login == "" || $senha == "" || $senha_conf == "" || $nome == "" || $cnpj == "" || $telefone == "" || $descricao = "" || $email == "" || $estado == "" || $cidade == "" || $bairro == "" || $rua == "" || $numero == "" || $complemento == "") {
    ...
    if ($rowcountcnpj == 1) {
        echo "<script language='javascript' type='text/javascript'>" . "alert('Esse cnpj já foi cadastrado!');" . "window.location.href='javascript:window.history.go(-1)'</script>";
    } else {
        ...
        $rowcount = mysqli_num_rows($mysqli->query($sql));
        if ($rowcount == 1) {
            echo "<script language='javascript' type='text/javascript'>" . "alert('Usuário cadastrado com sucesso!');" . "window.location.href='form_login.php'</script>";
        } else {
            echo "<script language='javascript' type='text/javascript'>" . "alert('Não foi possível cadastrar esse usuário');" . "window.location.href='javascript:window.history.go(-1)'</script>";
        }
    }
}
?>